...
def millisecOB(t, day):
    ...
    milliseconds = (date - datetime(2021, 10, day)) // timedelta(milliseconds=1)
...
def millisecM(t, day):
    ...
    milliseconds[np.where(f_yes == 1)] = (date - datetime(2021, 10, day)) // timedelta(milliseconds=1)
    date = pd.to_datetime(t[np.where(f_yes == 0)], format="%Y-%m-%dT%H:%M:%SZ")
    milliseconds[np.where(f_yes == 0)] = (date - datetime(2021, 10, day)) // timedelta(milliseconds=1)
    return milliseconds
...
def read_data(day):
    ...
    '''
    ORDERBOOK = 'LOB_ottobre21\LOB_Germany_Baseload_2022_202110' + day + '.csv'
    MSGBOOK = r'trade_ottobre2021\trades_Germany_Baseload_2022_202110' + day + '.csv'
    dataM = pd.read_csv(MSGBOOK)  # columns are already named
    dataM = dataM.loc[(millisecM(np.array(list(dataM.DateTime.values)), int(day)) > 10 * 60 * 60 * 1000) & (
            millisecM(np.array(list(dataM.DateTime.values)), int(day)) < 16 * 60 * 60 * 1000)]
    dataOB = pd.read_csv(ORDERBOOK)  # columns are already named
    dataOB = dataOB.loc[(millisecOB(dataOB.Datetime.values, int(day)) >= 9.9 * 60 * 60 * 1000) & (
            millisecOB(dataOB.Datetime.values, int(day)) <= 16.1 * 60 * 60 * 1000)]
    dataOB = dataOB.rename(columns={'Unnamed: 0': 'count'})
    Signs = estim_sign(dataM, dataOB, day)
...
def perm_Cart_Jaim(dataM, dataOB, day, freq=5 * 60 * 1000):
    '''
    ...
    for l in range(0, noint):
        n_buy[l] = sum(dataM.loc[(millisecM(M, day) > bound[l]) &
                                 (millisecM(M, day) < bound[l + 1]) & (
                                         dataM.AggressorAction == 'Buy')].Volume.values)  # volume of buy MO
        n_sell[l] = sum(dataM.loc[(millisecM(M, day) > bound[l]) &
                                  (millisecM(M, day) < bound[l + 1]) & (
    ...
    for i in range(0, noint):
        try:
            first_mid = 0.5 * (dataOB.loc[(millisecOB(OB, day) >= bound[i]) &
                                          (millisecOB(OB, day) < bound[i + 1])].AskPrice_0.values[0] +
                               dataOB.loc[(millisecOB(OB, day) >= bound[i]) &
                                          (millisecOB(OB, day) < bound[i + 1])].BidPrice_0.values[
                                   0])  # first price displayed after that time
            last_mid = 0.5 * (dataOB.loc[(millisecOB(OB, day) >= bound[i]) &
                                         (millisecOB(OB, day) < bound[i + 1])].AskPrice_0.values[-1] +
                              dataOB.loc[(millisecOB(OB, day) >= bound[i]) &
                                         (millisecOB(OB, day) < bound[i + 1])].BidPrice_0.values[
...
def perm_Has_Car_Jaim(dataOB, dataM, day):
    '''
    :param dataOB: Data set containing the order book
    :param dataM: Data set containing trades
    :param day: Plug in the day as an integer
    ...
    M = millisecM(np.array(list(dataM.DateTime.values)), int(day))
    OB = dataOB.Datetime.values
    midprice = np.zeros(len(M) + 1)
    ask = dataOB.loc[(millisecOB(OB, int(day)) < M[0])].AskPrice_0.values[-1]
    bid = dataOB.loc[(millisecOB(OB, int(day)) < M[0])].BidPrice_0.values[-1]
    midprice[0] = 0.5 * (ask + bid)
    for l in range(0, len(M)):
        try:
            ask = dataOB.loc[(millisecOB(OB, int(day)) > M[l]) &
                             (millisecOB(OB, int(day)) < M[l + 1])].AskPrice_0.values[0]
            bid = dataOB.loc[(millisecOB(OB, int(day)) > M[l]) &
                             (millisecOB(OB, int(day)) < M[l + 1])].BidPrice_0.values[0]
            midprice[l + 1] = 0.5 * (ask + bid)
        except:
            ask = dataOB.loc[(millisecOB(OB, int(day)) > M[l])].AskPrice_0.values[0]
            bid = dataOB.loc[(millisecOB(OB, int(day)) > M[l])].BidPrice_0.values[0]
            midprice[l + 1] = 0.5 * (ask + bid)
    rt = np.diff(midprice)
...
def temp_Cart_Jaim(dataOB):
    ...
    volumes = bids_cum.apply(lambda row: pd.Series(np.linspace(row[0], row[row.isnull() == False][-1], 10, dtype=int)),
                             axis=1)
...
def kappa_dynamics(dataOB):
    ...
    # MATRIX OF CUMULATIVE VOLUMES
    bids_cum = dataOB[['BidVolume_' + str(i) for i in range(10)]].apply(np.cumsum, axis=1, raw=True)
    volumes = bids_cum.apply(
        lambda row: pd.Series(np.arange(1, 1 + min(20, int(row[row.isnull() == False][-1])), dtype=int)), axis=1)
    # MATRIX OF INDEXES OF EXECUTION
    ind_ex = bids_cum.apply(lambda row: pd.Series(np.searchsorted(row, volumes.iloc[row.name], side='left')), axis=1)
    ind_ex = pd.DataFrame(ind_ex)
    tot = pd.DataFrame()
...
def temp_Glas(days):
    ...
    for day in days:
        ...
        OB = dataOB.Datetime.values
        dataOB = dataOB.rename(columns={'Unnamed: 0': 'count'})
        dataOB = dataOB.loc[(millisecOB(OB, int(day)) >= 9.9 * 60 * 60 * 1000) & (
                millisecOB(OB, int(day)) < 16.1 * 60 * 60 * 1000)]
        # dataOB.reset_index(drop=True, inplace=True)
        OB = dataOB.Datetime.values
        deltaT = np.diff((millisecOB(OB, int(day))))
        dataOB = dataOB[:-1]
        OB = OB[:-1]
        # freq = 5 * 60 * 1000  # 5minute
        ...
        for i in range(0, noint):
            try:
                ob = dataOB.loc[(millisecOB(OB, int(day)) >= bound[i]) &
                                (millisecOB(OB, int(day)) < bound[i + 1])]  # .AskPrice_0.values[0]
                Po = dataOB.loc[ob.index[0] - 1]
                wo = (millisecOB(ob.Datetime.values, int(day))[0] - bound[
                    i]) / freq  # first slice of time (is not a full interval)
                wN = (bound[i + 1] - millisecOB(ob.Datetime.values, int(day))[
...
def impact_Has_Alm(dataM, dataOB, day):
    '''
    :param dataM: Data set containing trades
    :param dataOB: Data set containing the order book
...
def estim_sign(dataM, dataOB, day):
    '''
    :param dataM: Data set containing trades
    :param dataOB: Data set containing the order book
    ...
    def get_sign(row):
        ...
        t = millisecM(t, int(day))
        OB = dataOB.Datetime.values
        ask = dataOB.loc[(millisecOB(OB, int(day)) < t)].AskPrice_0.values[-1]
        bid = dataOB.loc[(millisecOB(OB, int(day)) < t)].BidPrice_0.values[-1]
        spread = ask - bid
